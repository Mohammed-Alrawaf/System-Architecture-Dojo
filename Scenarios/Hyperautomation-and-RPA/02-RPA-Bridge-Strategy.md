...
## 1. Executive Summary
The business requires real-time, API-driven access to corporate registration data from a legacy government portal that lacks API capabilities. This initiative bridges modern RESTful API architecture with back-office Robotic Process Automation (RPA) using Blue Prism digital workers. The bots will navigate the HTML UI, scrape specific text fields, format them into a lightweight JSON payload (~5KB), and store them on-premise without human intervention.
...
* **Storage Layer:** All `RequestIDs` and lifecycle states are stored in a centralized, on-premise SQL table (`tbl_Request_State`).
...
* **Bot Write-Back:** Upon completion, the Blue Prism bot updates the SQL record to `COMPLETED` and writes the final JSON payload directly into the database using strict SQL transactions (`BEGIN TRAN`, `COMMIT`, `ROLLBACK`) to prevent partial data corruption if a network drop occurs.
...
* **Delivery Timeline:** Target Go-Live is 15 weeks (4 weeks procurement/vendor SOW, 3 weeks InfoSec audit, 6 weeks agile development, 2 weeks UAT).

## 4. Product Strategy & Monetization (Subscription Tiers)
...
## 5. Enterprise Risk Matrix & Security Mitigations
...
| **State Corruption** | Connection drops between RPA and SQL DB | Automated SQL Agent Job runs an "Orphan Sweep" every 10 minutes, moving 2-hour-old `PROCESSING` tasks to `FAILED`. Bot utilizes Exponential Backoff. |
...
## 6. Operational Knowledge & Support (Runbooks)
...
* **Queue Management:** Procedures for safely resetting "Stuck" or "Locked" queue items in Blue Prism native queues.
